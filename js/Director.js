...
export class Director{
  ...
  change(){
    // 死了一条命，切换无敌状态
    let wudi = this.dataStore.get('birds').wudi;
    if(wudi){
      setTimeout(()=>{
        console.log(wudi);
        this.dataStore.get('birds').wudi = false;
      },1000);
    }
  }
  ...
  check(){
    ...
    // 判断与天地相撞，没有无敌
    if((birds.birdsY[0]<0 || birds.birdsHeight[0]+birds.birdsY[0]>land.y) && !birds.wudi ){
      // 撞了，减命
      birds.life--;
      // 判断还有没有命
      if(birds.life==0){
    ...
    for(let i=0;i<pipes.length;i++){
      ...
      // 判断小鸟与某个水管是否撞击
      if(this.isStrike(birdBorder,pipeBorder) && !birds.wudi){
        // 撞了，减命
        birds.life--;
        // 判断还有没有命
        if (birds.life == 0) {
  ...
  run(){
    ...
    if(!this.isGameOver){
      ...
      if(this.dataStore.get('birds').wudi){
        let ctx = this.dataStore.ctx;
        ctx.fillStyle = 'rgba(150,100,50,0.7)';
        let birds = this.dataStore.get('birds');
        ctx.fillRect(birds.birdsX[0], birds.birdsY[0], birds.birdsWidth[0], birds.birdsHeight[0]);
      }
      // 循环运行
